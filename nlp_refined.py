    ...
    def store_data_by_battalion(self, url, title, body, preprocessed_text, sentiment, entities):
        ...
        for entity in entities:
            if entity['label'] == 'GPE':  # GPE: Geopolitical entity (states, cities, etc.)
                ...
                # Find which battalion this state belongs to
                for battalion, states in ndrf_battalions.items():
                    if state_name in states:
                        # Store data in the 'Regions' collection under the respective battalion
                        article_data = {
                            'url': url,
                            'title': title,
                            'body': body,
...
# 1. Use geotagged posts and cluster them by location for severity analysis.
# 2. Create interactive maps (using Leaflet.js or Mapbox) and heatmaps (Plotly or Seaborn).
# 3. Show the progression of flooding over time using graphs and time series.
# 4. Generate real-time text alerts for critical areas.
